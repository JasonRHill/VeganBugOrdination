---
title: "NMDS for Reference streams-3% removed"
...
dataList <- removeRareTaxa(Refbugs, # dataset to analyze, can filter on whatever you want beforehand (e.g. ref, stress, play with ecoregions)
                           threshold = 3, # drop taxa below 5% occurrence in dataset 
...
NMDSthree<-dataList$`Benthics Slim`
...
bugsnms<-NMDSthree%>%
  select("StationID", "Collection.Date", 6:144)
...
The data was log10+1 transformed. Rare taxa (<=3%) were removed. </font>
...
```{r bugnms, echo=FALSE}
NMDSbug <-metaMDS (NMDSthree[,6:144],k=3, trymax=1000)
...
Season.mrpp<- mrpp(bugsnms[,6:144], group=samplescoresenv$Season, distance="bray")
...
Ecoregion.mrpp<- mrpp(bugsnms[,6:144], group= samplescoresenv$US_L3NAME, distance="bray")
...
Basin.mrpp<- mrpp(bugsnms[,6:144], group=samplescoresenv$Basin_Code, distance="bray" )
...
#Ecoregion.mrpp<- with(Bugenv, mrpp(bugsnms[,2:126], group= US_L3NAME, distance="bray"))
admin.mrpp<- mrpp(bugsnms[,6:144], group=samplescoresenv$ASSESS_REG, distance="bray" )
...
Sample.mrpp<-mrpp(bugsnms[,6:144], group=samplescoresenv$Gradient, distance="bray" )
...
Stream.mrpp<-mrpp(bugsnms[,6:144], group=samplescoresenv$Order, distance="bray" )
...
Standard.mrpp<- mrpp(bugsnms[,6:144], samplescoresenv$WQS_CLASS, distance="bray" )
