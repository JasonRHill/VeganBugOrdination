---
title: "NMDS for Reference streams"
...
bug.spp.fit <- envfit(NMDSbug, samplescoresenv[ , 7:34], permutations = 999, na.rm=TRUE) 
...
ggplot(data = samplescoresenv, aes(x = NMDS2, y = NMDS3)) + 
     geom_point(data = samplescoresenv, aes(colour =Season), size = 5, alpha = 0.5) + 
     scale_colour_manual(values = c("orange", "steelblue", "black")) + 
  annotate("text", x=0.8, y=1.3, label= "Stress= 0.16", size=6)+
     theme(axis.title = element_text(size = 14, face = "bold", colour = "grey30"), 
...
ggplot(data = samplescoresenv, aes(x = NMDS1, y = NMDS3)) + 
     geom_point(data = samplescoresenv, aes(colour =Season), size = 5, alpha = 0.5) + 
     scale_colour_manual(values = c("orange", "steelblue", "black")) + 
  annotate("text", x=0.8, y=1.3, label= "Stress= 0.16", size=6)+
     theme(axis.title = element_text(size = 14, face = "bold", colour = "grey30"), 
...
Season.mrpp<- mrpp(bugsnms[,6:119], group=samplescoresenv$Season, distance="bray")
...
Ecoregion.mrpp<- mrpp(bugsnms[,6:119], group= samplescoresenv$US_L3NAME, distance="bray")
...
Basin.mrpp<- mrpp(bugsnms[,6:119], group=samplescoresenv$Basin_Code, distance="bray" )
...
#Ecoregion.mrpp<- with(Bugenv, mrpp(bugsnms[,2:126], group= US_L3NAME, distance="bray"))
admin.mrpp<- mrpp(bugsnms[,6:119], group=samplescoresenv$ASSESS_REG, distance="bray" )
...
Sample.mrpp<-mrpp(bugsnms[,6:119], group=samplescoresenv$Gradient, distance="bray" )
...
ggplot(data = samplescoresenv, aes(x = NMDS1, y = NMDS2)) + 
     geom_point(data = samplescoresenv, aes(colour = Bioregion), size = 5, alpha = 0.8) + 
...
bioregion.mrpp<- mrpp(bugsnms[,6:119], group= samplescoresenv$Bioregion, distance="bray")
...
ggplot(data = samplescoresenv, aes(x = NMDS1, y = NMDS2)) + 
     geom_point(data = samplescoresenv, aes(colour = Bioregion, shape=Season), size = 4, alpha = 0.8) + 
     scale_colour_manual(values = c("darkred", "bisque","darkorchid2" )) +
  ...
  annotate("text", x=0.8, y=1.3, label= "Stress= 0.16", size=6)+
     ...
     legend.title = element_text(size = 15, face = "bold", colour = "grey30"), 
     legend.text = element_text(size = 15, colour = "grey30")) +
     labs(colour = "Bioregion", shape= "Season")
...
Stream.mrpp<-mrpp(bugsnms[,6:119], group=samplescoresenv$Order, distance="bray" )
...
Standard.mrpp<- mrpp(bugsnms[,6:119], samplescoresenv$WQS_CLASS, distance="bray" )
...
Trout.mrpp<- mrpp(bugsnms[,6:119], samplescoresenv$WQS_TROUT, distance="bray" )
...
Based on the observations from the Ecoregion plots, we removed the coastal streams to better assess patterns in the non-coastal streams. The plots in the "Non-Coastal Reference Ordinations" document represent streams within all ecoregions except Mid-Atlantic Coastal Plains and Southeastern Plains ecoregions. 
...
#noncoast <- envfit(NMDSbug_noncoast, samplescoresenv_noncoast[ , 7:33] , permutations = 999, na.rm=TRUE) 
...
ggplot(data = samplescoresenv_noncoast, aes(x = NMDS1, y = NMDS2)) + 
     geom_point(data = samplescoresenv_noncoast, aes(colour = Bioregion, shape=Season), size = 4, alpha = 0.8) + 
     scale_colour_manual(values = c("darkred", "bisque","darkorchid2" )) +
  scale_shape_manual(values = c(0, 3, 16))+
  annotate("text", x=0.8, y=1.3, label= "Stress= 0.18", size=6)+
     theme(axis.title = element_text(size = 14, face = "bold", colour = "grey30"), 
